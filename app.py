...
FIGURE_GUIDE = [{'Figure': 1,
  ...
  'Tab': 'Overview',
  'Setting': 'Month; panel a: total orders; panel b: product sales'},
 {'Figure': 2,
  'Title': 'Commercial performance over time',
  'Tab': 'Overview',
  'Setting': 'Month; panel a: average order value; panel b: items per order'},
 {'Figure': 3,
 ...
 {'Figure': 8,
  ...
  'Setting': 'Default'},
 {'Figure': 9,
  'Title': 'Low-rating rate for on-time or early versus late deliveries',
  'Tab': 'Overview',
  'Setting': 'Default'},
 {'Figure': 10,
 ...
 {'Figure': 11,
  'Title': 'Mean review score by shipping duration',
  'Tab': 'Delivery correlations',
  'Setting': 'Carrier-to-customer shipping duration'},
 {'Figure': 12,
  'Title': 'Delivery performance and reviews by customer–seller distance',
  'Tab': 'Delivery correlations',
  'Setting': 'Distance sextiles; panel a: delivery days; panel b: review score and late rate'},
 {'Figure': 13,
  'Title': 'Delivery performance and reviews by product category',
  'Tab': 'Delivery correlations',
  'Setting': 'Top 13 categories by mean delivery time; panels a and b'},
 {'Figure': 14,
  'Title': 'Seller order volume and late-delivery rate',
  'Tab': 'Delivery correlations',
  'Setting': 'Default'},
 {'Figure': 15,
  'Title': 'Delivery performance and reviews by payment method',
  'Tab': 'Delivery correlations; panel a repeated in Candidate 1',
  'Setting': 'Primary payment method; panel a: delivery days; panel b: review score and late rate'},
 {'Figure': 16,
  'Title': 'Lead-time breakdown by payment method',
  'Tab': 'Delivery correlations; repeated in Candidate 1',
  'Setting': 'Default'},
 {'Figure': 17,
 ...
 {'Figure': 19,
  ...
  'Setting': 'Median'},
 {'Figure': 20,
  'Title': 'Purchase day and hour heatmap',
  'Tab': 'Operational Capacity; repeated in Candidate 1',
  'Setting': 'Mean delivery days or Order volume; Capacity: expand More detail on purchase timing'},
 {'Figure': 21,
  'Title': 'Delivery-stage duration by purchase weekday',
  'Tab': 'Operational Capacity; repeated in Candidate 1',
  'Setting': 'Panels a–c: processing, handling and shipping; Capacity: expand More detail on '
             'purchase timing'},
 {'Figure': 22,
 ...
 {'Figure': 24,
  ...
  'Setting': 'Mean'},
 {'Figure': 25,
  'Title': 'Brazil order and seller map',
  'Tab': 'Mapped orders and sellers',
  'Setting': 'Select the relevant map layers'},
 {'Figure': 26,
 ...
 {'Figure': 29,
  ...
  'Setting': 'Default'},
 {'Figure': 30,
  'Title': 'Low-rating rate across five delivery-timing bands',
  'Tab': 'Candidate 2',
  'Setting': 'Default'},
 {'Figure': 31,
  'Title': 'Low-rating rate by order complexity',
  'Tab': 'Candidate 2',
  'Setting': 'Panel a: number of items; panel b: number of sellers'},
 {'Figure': 32,
  'Title': 'Pearson correlations with mean review score',
  'Tab': 'Candidate 2',
  'Setting': 'Default'}]
FIGURE_TITLES = {row["Figure"]: row["Title"] for row in FIGURE_GUIDE}
FIGURE_PANELS = {'1a': 'Total orders',
 '1b': 'Product sales',
 '2a': 'Average order value',
 '2b': 'Items per order',
 '12a': 'Mean delivery days by customer–seller distance',
 '12b': 'Mean review score and late rate by distance',
 '13a': 'Mean delivery days by product category',
 '13b': 'Mean review score by product category',
 '15a': 'Mean delivery days by payment method',
 '15b': 'Mean review score and late rate by payment method',
 '21a': 'Processing duration by purchase weekday',
 '21b': 'Handling duration by purchase weekday',
 '21c': 'Shipping duration by purchase weekday',
 '31a': 'Low-rating rate by number of items',
 '31b': 'Low-rating rate by number of sellers'}


def figure_heading(number: int, repeat_from: str | None = None) -> None:
    """Number unique figures once and retain that number for repeated views."""
    st.markdown(f"### Figure {number}. {FIGURE_TITLES[number]}")
    if repeat_from:
        st.caption(f"Repeated from {repeat_from}; the figure reference is the same.")


def panel_heading(reference: str, repeat_from: str | None = None) -> None:
    """Give each panel a reference that can be cited independently."""
    st.markdown(f"#### Figure {reference}. {FIGURE_PANELS[reference]}")
    if repeat_from:
        st.caption(f"Repeated from {repeat_from}; the panel reference is the same.")
...
def visible_map_html(path: Path) -> str:
    """Initialise Folium only after its tab has a non-zero size."""
    html = path.read_text(encoding="utf-8")
    script_start = html.rfind("<script>")
    script_end = html.find("</script>", script_start)
    if script_start < 0 or script_end < 0:
        return html
    map_script = html[script_start + len("<script>"):script_end]
    deferred_script = """
    (() => {
        function initialiseMap() {
    """ + map_script + """
        }
        const container = document.querySelector('.folium-map');
        if (!container) return;
        let initialised = false;
        const observer = new ResizeObserver(() => {
            if (!initialised && container.clientWidth > 0 && container.clientHeight > 0) {
                initialised = true;
                observer.disconnect();
                initialiseMap();
            }
    ...
    """
    return html[:script_start + len("<script>")] + deferred_script + html[script_end:]
...
st.caption("Start with the overview, explore supporting relationships, capacity trends and the map, then compare the two candidate problems. Every figure is numbered by its first appearance. Panels use a/b/c labels; repeated views retain their original reference.")
...
with capacity_tab:
    ...
    with st.expander("More detail on purchase timing"):
        ...
        for panel_letter, col, stage_key in zip("abc", weekday_cols, stage_key_order):
            stage_data = weekday_stage.loc[weekday_stage["stage_key"] == stage_key]
            with col:
                panel_heading(f"21{panel_letter}")
...
with ratings_tab:
    ...
    with st.container():
        figure_heading(31)
        for panel_letter, dimension, column in zip(
            "ab", ["Number of items", "Number of sellers"], st.columns(2)
        ):
            with column:
    ...
    with st.container():
        ...
        st.markdown(
            ...
            "rather than the binary latest-review target in Figures 30–31. Distance uses "
...
with problem1_tab:
    ...
    figure_heading(20, repeat_from="Operational Capacity → More detail on purchase timing")
    ...
    figure_heading(21, repeat_from="Operational Capacity → More detail on purchase timing")
    ...
    for panel_letter, col, stage_key in zip("abc", p1_weekday_cols, p1_stage_key_order):
    ...
    panel_heading("15a", repeat_from="Delivery correlations")
    ...
    figure_heading(16, repeat_from="Delivery correlations")
